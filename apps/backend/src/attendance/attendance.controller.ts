...
export class AttendanceController { constructor(private readonly service: AttendanceService) {}
  ...
  @Get('students') @RequirePermissions('ATTENDANCE_VIEW') students(@Req() req: any, @Query('date') date?: string, @Query('classId') classId?: string, @Query('sectionId') sectionId?: string) { return this.service.listStudents(req.user.schoolId, date, classId, sectionId); }
