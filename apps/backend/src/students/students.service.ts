...
export class StudentsService {
  ...
  async enroll(studentId: string, schoolId: string | null, data: EnrollStudentDto) {
    ...
    if (data.sectionId) { const section = await this.prisma.section.findFirst({ where: { id: data.sectionId, schoolId, classId: data.classId } }); if (!section) throw new BadRequestException('Section must belong to the selected class and school.'); }
  ...
  async updateEnrollment(studentId: string, enrollmentId: string, schoolId: string | null, data: UpdateStudentEnrollmentDto) {
    ...
    const classId = data.classId ?? enrollment.classId; const sectionId = data.sectionId !== undefined ? data.sectionId : data.classId !== undefined ? null : enrollment.sectionId;
    ...
    if (sectionId) { const section = await this.prisma.section.findFirst({ where: { id: sectionId, schoolId, classId } }); if (!section) throw new BadRequestException('Section must belong to the selected class.'); }
