...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (loading) return; if (!session && !['/login'].includes(path)) router.replace('/login'); else if (session?.user.mustChangePassword && path !== '/change-password') router.replace('/change-password'); else if (session && !session.user.mustChangePassword && ['/login', '/change-password'].includes(path)) router.replace('/dashboard'); }, [loading, path, router, session]);
